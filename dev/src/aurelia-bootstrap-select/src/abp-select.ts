...
import {OptionalBindingBehavior} from './optional-binding-behavior';
...
export class AbpSelectCustomElement {
  ...
  // variables
  private _originalSelectedIndexes;
  private _originalSelectedObjects;
  domElm: any;

  constructor(private elm: any, private utilService: UtilService) {
    this.elm = elm;
  ...
  findItems(collection, newValue, objectKey) {
    ...
    for (let searchItem of searchingItems) {
      let searchFilter = this.utilService.isObject(searchItem) ? searchItem[objectKey] : searchItem;
      let foundItem = collection.find(item => {
        // for comparison, we're using == mostly because indexes are passed as string because of html
        return this.utilService.isObject(item) ? item[objectKey] == searchFilter : item == searchFilter;
      });
      if (foundItem) {
        selection.indexes.push(this.utilService.isObject(foundItem) ? foundItem[objectKey] : foundItem);
  ...
  isEmptySelection(selection) {
    ...
    return (!selection.items || selection.items.length === 0) && (!selection.indexes || selection.indexes.length === 0);
  }
  ...
  renderSelection(selection) {
    ...
    } else if (this.utilService.parseBool(this.emptyOnNull) && this.isEmptySelection(selection)) {
  ...
  selectedItemChanged(newValue, oldValue) {
    if (!this.utilService.isEqual(newValue, oldValue)) {
      ...
      if ((!this.utilService.parseBool(this.emptyOnNull) && !this.multiple) || this.isNotEmptySelection(selection)) {
        if (selection.indexes.length > 0) {
          ...
          const selectedValue = (this.utilService.isObject(this.collection[0]) ? this.collection[0][this.objectKey] : this.collection[0]);
          this.selectedValue = (!this.multiple && Array.isArray(selectedValue)) ? selectedValue[0] : selectedValue;
        }
      }
  ...
  selectedValueChanged(newValue, oldValue) {
    if (!this.utilService.isEqual(newValue, oldValue)) {
      ...
      } else if (this.utilService.parseBool(this.emptyOnNull) && !this.multiple) {
  ...
  watchOnLoadedToRenderPreSelection() {
    this.domElm.on('loaded.bs.select', (e) => {
      ...
      if (selection.indexes) {
        ...
        this.selectedValue = (this.utilService.isObject(this.collection[0]) ? this.collection[0][this.objectKey] : this.collection[0]);
      }
